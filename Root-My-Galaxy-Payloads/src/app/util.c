...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
  ...
  if (payload_mode == PAGE_PAYLOAD_FOPS) {
    ...
    /* The physical route addresses the enclosing miscdevice object.  The
     * fops pointer field inside that object is handled by the CFI stage. */
  ...
  if (payload_mode == PAGE_PAYLOAD_FOPS) {
    ...
    fake_parent = misc_fops_data_addr() - 8;
